...
class ImprovedTangentNEB(OriginalNEB):
    ...
    def get_parallel_forces(self, tangent, pos_p, pos_m, **kwargs):
        forces_parallel = (self.k[1:] * np.linalg.norm(pos_p, axis=(1, 2))) - (
            self.k[:-1] * np.linalg.norm(pos_m, axis=(1, 2))
        )
        ...
        for i in range(1, self.nimages - 1):
            if energies[i + 1] > energies[i] and energies[i] > energies[i - 1]:
                ...
                tangent[i - 1] = (pos_p[i - 1] * min(energy_dif)) + (
                    pos_m[i - 1] * max(energy_dif)
                )
            else:
                tangent[i - 1] = (
                    pos_p[i - 1] / np.linalg.norm(pos_p[i - 1])
                ) + (pos_m[i - 1] / np.linalg.norm(pos_m[i - 1]))
