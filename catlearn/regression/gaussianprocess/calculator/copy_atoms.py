...
class StoredDataCalculator(Calculator):
    """
    A special calculator that store the data (results) of a single configuration.
    ...
    def get_property(self,name,atoms=None,allow_calculation=True):
        if atoms is None:
            atoms=self.atoms
        # Raise an error if the property does not exist or the configuaration has changed
