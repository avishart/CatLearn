...
class EnsembleClustering(EnsembleModel):
    ...
    def optimize(self,features,targets,retrain=True,hp=None,pdis=None,verbose=False,**kwargs):
        ...
        # If only one model is used
        if self.n_models==1:
            sol=self.model_optimization(self.model,features,targets,retrain=retrain,hp=hp,pdis=pdis,verbose=verbose,**kwargs)
            sols.append(sol)
            self.models.append(self.model)
            return sols
        # If multiple models are used
        for ki in range(self.n_models):
            model=self.model.copy()
            sol=self.model_optimization(model,cdata[ki][0],cdata[ki][1],retrain=retrain,hp=hp,pdis=pdis,verbose=verbose,**kwargs)
            sols.append(sol)
            self.models.append(model)         
        return sols
