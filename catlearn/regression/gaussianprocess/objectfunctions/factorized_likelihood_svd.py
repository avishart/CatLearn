...
class FactorizedLogLikelihoodSVD(Object_functions):
    ...
    def function(self,theta,GP,parameters,X,Y,prior=None,jac=False,dis_m=None):
        ...
        D,U,Y_p,VYUTY,KXX,n_data=self.get_eig(GP,X,Y,dis_m)
        noise,nlp=self.maximize_noise(GP,X,Y_p,parameters_set,parameters,prior,VYUTY,D,n_data)
        if jac==False:
            return nlp
        # Derivatives
        nlp_deriv=np.array([])
        D_n=D+np.exp(2*noise)
        prefactor2=np.mean(VYUTY/D_n)
    ...
    def get_eig_ll(self,noise,hp,parameters_set,parameters,prior,VYUTY,D,n_data):
        " Calculate log-likelihood from SVD "
        D_n=D+np.exp(2*noise)
        prefactor2=np.mean(VYUTY/D_n)
    ...
    def maximize_noise(self,GP,X,Y,parameters_set,parameters,prior,VYUTY,D,n_data):
        " Find the maximum noise "
        noises=self.make_noise_list(GP,X,Y)
        args_ll=(GP.hp.copy(),parameters_set,parameters,prior,VYUTY,D,n_data)
