...
class FactorizedGPP(FactorizedLogLikelihood):
    def __init__(self,get_prior_mean=False,modification=False,ngrid=80,bounds=None,noise_optimizer=None,**kwargs):
        ...
        """
        super().__init__(get_prior_mean=get_prior_mean,
    ...
    def function(self,theta,parameters,model,X,Y,pdis=None,jac=False,**kwargs):
        ...
        noise,gpp_v=self.maximize_noise(parameters,model,X,Y,pdis,hp,U,UTY,D,n_data)
        if jac:
            deriv=self.derivative(hp,parameters_set,model,X,KXX,D,U,Y_p,UTY,noise,pdis,n_data,**kwargs)
            self.update_solution(gpp_v,theta,hp,model,jac=jac,deriv=deriv,noise=noise,UTY=UTY,U=U,D=D,n_data=n_data)
            return gpp_v,deriv
        self.update_solution(gpp_v,theta,hp,model,jac=jac,noise=noise,UTY=UTY,U=U,D=D,n_data=n_data)
        return gpp_v
    ...
    def get_eig_fun(self,noise,hp,pdis,U,UTY,D,n_data,**kwargs):
        ...
        prefactor=0.5*np.log(np.mean((np.matmul(UDn,UTY).reshape(-1)**2)/K_inv_diag))
        gpp_v=1-np.mean(np.log(K_inv_diag))+2.0*prefactor+np.log(2.0*np.pi)
        if pdis is not None:
    ...
    def get_all_eig_fun(self,noises,hp,pdis,U,UTY,D,n_data,**kwargs):
        ...
        prefactor=0.5*np.log(np.mean((np.matmul(UDn,UTY).reshape((len(noises),n_data))**2)/K_inv_diag,axis=1))
        gpp_v=1-np.mean(np.log(K_inv_diag),axis=1)+2.0*prefactor+np.log(2.0*np.pi)
        if pdis is not None:
            hp['prefactor']=prefactor.reshape(-1,1)
            hp['noise']=noises
        return gpp_v-self.logpriors(hp,pdis,jac=False)/n_data
        
    def maximize_noise(self,parameters,model,X,Y,pdis,hp,U,UTY,D,n_data,**kwargs):
    ...
    def update_solution(self,fun,theta,hp,model,jac=False,deriv=None,noise=None,UTY=None,U=None,D=None,n_data=None,**kwargs):
        """
