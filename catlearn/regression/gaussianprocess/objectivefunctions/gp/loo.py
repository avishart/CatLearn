...
class LOO(ObjectiveFuction):
    def __init__(self,get_prior_mean=False,use_analytic_prefactor=True,**kwargs):
        """ 
    ...
    def function(self,theta,parameters,model,X,Y,pdis=None,jac=False,**kwargs):
        ...
        if jac:
            deriv=self.derivative(hp,parameters_set,model,X,KXX,KXX_inv,K_inv_diag,coef_re,co_Kinv,n_data,pdis,**kwargs)
            self.update_solution(loo_v,theta,hp,model,jac=jac,deriv=deriv,coef_re=coef_re,K_inv_diag=K_inv_diag,co_Kinv=co_Kinv)
            return loo_v,deriv
        self.update_solution(loo_v,theta,hp,model,jac=jac,coef_re=coef_re,K_inv_diag=K_inv_diag,co_Kinv=co_Kinv)
        return loo_v
    
    def derivative(self,hp,parameters_set,model,X,KXX,KXX_inv,K_inv_diag,coef_re,co_Kinv,n_data,pdis,**kwargs):
        loo_deriv=np.array([])
    ...
    def update_arguments(self,get_prior_mean=None,use_analytic_prefactor=None,**kwargs):
        """
    ...
    def update_solution(self,fun,theta,hp,model,jac=False,deriv=None,coef_re=None,K_inv_diag=None,co_Kinv=None,**kwargs):
        """
        ...
        than the input since it is optimized analytically if use_analytic_prefactor=True.
        """
        if fun<self.sol['fun']:
