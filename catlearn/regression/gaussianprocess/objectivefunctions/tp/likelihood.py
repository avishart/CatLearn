...
class LogLikelihood(ObjectiveFuction):
    ...
    def function(self,theta,parameters,model,X,Y,pdis=None,jac=False,**kwargs):
        ...
        nlp=np.sum(np.log(np.diagonal(L)))+0.5*(2.0*a+n_data)*np.log(ycoef)
        nlp=nlp-self.logpriors(hp,pdis,jac=False)
        if jac:
            return nlp,self.derivative(hp,parameters_set,model,X,Y_p,KXX,L,low,coef,a,b,ycoef,n_data,pdis,**kwargs)   
        return nlp
    
    def derivative(self,hp,parameters_set,model,X,Y_p,KXX,L,low,coef,a,b,ycoef,n_data,pdis,**kwargs):
        nlp_deriv=np.array([])
