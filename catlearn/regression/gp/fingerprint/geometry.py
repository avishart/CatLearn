...
def get_all_distances(
    ...
    **kwargs,
    ...
    if masked is None:
        masked = np.array(
            list(set(np.arange(len(atoms))).difference(set(not_masked)))
        )
    # Make indicies
...
def get_inverse_distances(
    ...
    **kwargs,
    ...
    """
    Get the inverse cartesian distances between the atomes.
    The derivatives can also be obtained.
    """
    # If a not masked list is given all atoms is treated to be not masked
    if not_masked is None:
        not_masked = np.arange(len(atoms))
    ...
    if use_covrad:
        ...
        if len(masked):
            covrad = np.concatenate(
                [
                    (covrad[masked] + covrad[not_masked, None]).reshape(-1),
                    covrad[nmj_ind] + covrad[nmi_ind],
                ],
                axis=0,
            )
        else:
