...
class InvDistances(Fingerprint):
    ...
    def element_setup(
        ...
        **kwargs,
        ...
        bools = np.all(
            np.array(combis).reshape(-1, 1, 2) == unique_combis, axis=2
        )
        if not nm_bool:
    ...
    def get_indicies(
        ...
        **kwargs,
        ...
        indicies_nm_m = np.arange(n_nm_m, dtype=int).reshape(
            n_nmasked, n_masked
        )
        # Make the indicies of not fixed and fixed atoms interactions
        indicies_nm_nm = np.zeros((n_nmasked, n_nmasked), dtype=int)
        indicies_nm_nm[nmi, nmj] = indicies_nm_nm[nmj, nmi] = np.arange(
            n_nm_m, n_total, dtype=int
        )
        return indicies_nm_m, indicies_nm_nm

    def get_indicies_combination(
        self,
        ci,
        cj,
        nmasked_indicies,
        ...
        **kwargs,
        ...
        """
        Get all the indicies in the fingerprint for
        the specific combination of atom types.
        """
        indicies_comb = []
        i_nm_ci = nmasked_indicies[ci].reshape(-1, 1)
        if ci == cj:
            indicies_comb = list(
                indicies_nm_m[i_nm_ci, masked_indicies[cj]].reshape(-1)
            )
            ind_prod = np.array(
                list(itertools.combinations(nmasked_indicies[ci], 2))
            )
            if len(ind_prod):
                indicies_comb = indicies_comb + list(
                    indicies_nm_nm[ind_prod[:, 0], ind_prod[:, 1]]
                )
        else:
            indicies_comb = list(
                indicies_nm_m[i_nm_ci, masked_indicies[cj]].reshape(-1)
            ...
            indicies_comb = indicies_comb + list(
                indicies_nm_m[
                    nmasked_indicies[cj].reshape(-1, 1), masked_indicies[ci]
                ].reshape(-1)
            )
            indicies_comb = indicies_comb + list(
                indicies_nm_nm[i_nm_ci, nmasked_indicies[cj]].reshape(-1)
            )
