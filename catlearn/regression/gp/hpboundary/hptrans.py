...
class VariableTransformation(HPBoundaries):
    def __init__(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            eps : float
                The first value of a grid in the variable transformed
    ...
    def get_variable_transformation_parameters(
        ...
        **kwargs,
        ...
        if array:
            var_dict_array = {}
            var_dict_array["mean"] = np.concatenate(
                [self.var_dict[para]["mean"] for para in parameters_set],
                axis=0,
            )
            var_dict_array["std"] = np.concatenate(
                [self.var_dict[para]["std"] for para in parameters_set], axis=0
            )
    ...
    def get_hp(
        ...
        **kwargs,
        ...
        if transformed:
            ...
            parameters_set = self.get_parameters_set(parameters=parameters)
            return {
                para: np.full((len(self.bounds_dict[para])), 0.50)
                for para in parameters_set
            }
    ...
    def make_lines(
        ...
        **kwargs,
        ...
        if transformed:
            return [
                np.linspace(self.eps, 1.00 - self.eps, ngrid[i])
    ...
    def make_single_line(
        self, parameter, ngrid=80, i=0, transformed=False, **kwargs
    ):
        """
        Make grid in each dimension of the hyperparameters from
        the boundary conditions.
    ...
    def sample_thetas(
        self, parameters=None, npoints=50, transformed=False, **kwargs
    ):
        """
        ...
        var_dict_array = self.get_variable_transformation_parameters(
            parameters=parameters, array=True
        )
        for i, (vt_mean, vt_std) in enumerate(
