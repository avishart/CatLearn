...
class Variable_Transformation:
    ...
    def transf_para(self,parameters,TP,X,Y,use_bounds=True,s=0.14):
        " Make a dictionary of the parameters for the variable transformation "
        parameters_set=sorted(parameters)
        if use_bounds:
            self.transf_para_bounds(parameters,parameters_set,TP,X,Y,s=s)
        else:
