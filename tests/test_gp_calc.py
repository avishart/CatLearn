...
class TestGPCalc(unittest.TestCase):
    ...
    def test_predict(self):
        ...
        from catlearn.regression.gaussianprocess.calculator import Database,DatabaseDistance,DatabaseHybrid,DatabaseMin,DatabaseRandom,DatabaseLast,MLModel,MLCalculator
        # Create the data set
        ...
        error_list=[0.00166,0.00166,0.00359,0.00011,0.00313,0.02942,0.000002,0.000002,0.000002,0.00943,0.01270,0.02064]
        ...
        for index,(data,use_fingerprint,data_kwarg) in enumerate(data_kwargs):
            with self.subTest(data=data,use_fingerprint=use_fingerprint,data_kwarg=data_kwarg):
                ...
                gp=GaussianProcess(hp=dict(length=2.0),use_derivatives=use_derivatives,kernel=SE(use_derivatives=use_derivatives,use_fingerprint=use_fingerprint),hpfitter=hpfitter)
                # Make the fingerprint
                fp=Cartesian(reduce_dimensions=True,use_derivatives=use_derivatives,mic=True)
                # Set up the database
                database=data(fingerprint=fp,reduce_dimensions=True,use_derivatives=use_derivatives,negative_forces=True,use_fingerprint=use_fingerprint,**data_kwarg)
                # Define the machine learning model
                mlmodel=MLModel(model=gp,database=database,optimize=True,optimize_kwargs={},baseline=None)
