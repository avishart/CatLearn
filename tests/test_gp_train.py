...
class TestGPTrainPredict(unittest.TestCase):
    ...
    def test_predict1(self):
        ...
        gp.train(x_tr,f_tr)
        # Predict the energy
        ypred,var,var_deriv=gp.predict(x_te,get_variance=False,get_derivatives=False,include_noise=False)
    ...
    def test_predict_var_n(self):
        ...
        gp.train(x_tr,f_tr)
        # Predict the energies and uncertainties
        ypred,var,var_deriv=gp.predict(x_te,get_variance=True,get_derivatives=False,include_noise=True)
        # Test the prediction energy errors
        error=calculate_rmse(f_te[:,0],ypred[:,0])
        self.assertTrue(abs(error-1.75102)<1e-4)
    ...
    def test_predict_derivatives(self):
        ...
        ypred,var,var_deriv=gp.predict(x_te,get_variance=True,get_derivatives=True,include_noise=False)
