...
class TestTPOptimizer(unittest.TestCase):
    ...
    def test_random(self):
        ...
        test_kwargs=[dict(bounds=None,hptrans=True,use_bounds=True),
                     ...
                     dict(bounds=np.array([[-3.0,3.0],[-8.0,0.0]]),hptrans=False,use_bounds=False)]
    ...
    def test_grid(self):
        ...
        test_kwargs=[dict(optimize=False,bounds=None,hptrans=True,use_bounds=True),
                     ...
                     dict(optimize=True,bounds=np.array([[-3.0,3.0],[-8.0,0.0]]),hptrans=False,use_bounds=False)]
        # Make a list of the solution values that the test compares to
    ...
    def test_line(self):
        ...
        test_kwargs=[dict(optimize=False,bounds=None,hptrans=True,use_bounds=True),
                     ...
                     dict(optimize=True,bounds=np.array([[-3.0,3.0],[-8.0,0.0]]),hptrans=False,use_bounds=False)]
        # Make a list of the solution values that the test compares to
        sol_list=[{'fun':502.249,'x':np.array([2.00,-70.98])},
    ...
    def test_basin(self):
        ...
        # Test the solution deviation
        self.assertTrue(abs(sol['fun']-502.207)<1e-2) 
        self.assertTrue(np.linalg.norm(sol['x']-np.array([1.97,-14.81]))<1e-2)
    ...
    def test_annealling(self):
        ...
        test_kwargs=[dict(bounds=None,use_bounds=True),
                     ...
                     dict(bounds=np.array([[-3.0,3.0],[-8.0,0.0]]),use_bounds=True)]
        # Make a list of the solution values that the test compares to
        sol_list=[{'fun':499.866,'x':np.array([2.55,-2.00])},
                  {'fun':499.866,'x':np.array([2.55,-2.00])},
                  {'fun':499.866,'x':np.array([2.55,-2.00])}]
    ...
    def test_line_search_scale(self):
        ...
        test_kwargs=[(dict(local_run=run_golden,bounds=None,hptrans=False,use_bounds=False),dict(fun_list=calculate_list_values,tol=1e-5,optimize=False,multiple_min=False)),
                     ...
                     (dict(local_run=fine_grid_search,bounds=np.array([[-3.0,3.0],[-8.0,0.0]]),hptrans=True,use_bounds=True),dict(fun_list=calculate_list_values,tol=1e-5,loops=3,iterloop=80,optimize=True,multiple_min=True)),
        ...
        # Make a list of the solution values that the test compares to
        sol_list=[{'fun':499.877,'x':np.array([2.53,-1.98])},
                  {'fun':500.075,'x':np.array([2.46,-1.93])},
                  {'fun':499.867,'x':np.array([2.56,-2.00])},
                  {'fun':499.866,'x':np.array([2.55,-2.00])},
